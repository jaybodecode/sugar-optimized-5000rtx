...
# Print header
Write-Host ("{0,-19} | {1,8} | {2,5} | {3,9} | {4,8} | {5,8}" -f "Timestamp", "VRAM", "GPU%", "WSL2 RAM", "Delta MB", "Total RAM")
Write-Host ("{0,-19}-+-{1,8}-+-{2,5}-+-{3,9}-+-{4,8}-+-{5,8}" -f ("-"*19), ("-"*8), ("-"*5), ("-"*9), ("-"*8), ("-"*8))

# Write CSV header if logging
if ($LogFile -ne "") {
    "Timestamp,VRAM_GB,GPU_Util_Pct,WSL2_RAM_GB,RAM_Delta_MB,Total_RAM_GB" | Out-File -FilePath $LogFile -Encoding UTF8
}
...
while ($true) {
    ...
    # Print to console
    Write-Host ("{0,-19} | {1,6:F2} GB | {2,4}% | {3,7:F2} GB | {4,8} | {5,6:F1} GB" -f $Timestamp, $VRAM_GB, $GPU_Util, $WSLMemGB, $DeltaStr, $UsedRAMGB)
