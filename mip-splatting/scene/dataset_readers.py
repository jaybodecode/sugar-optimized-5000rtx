...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        log.print_progress(f"Reading camera {idx+1}/{len(cam_extrinsics)}")
        ...
        # Create lightweight placeholder with just .size attribute (no pixel data loaded)
        # This avoids loading all 185 images into RAM during scene initialization
        ...
        image = LazyImagePlaceholder(width, height, image_path)
...
def readColmapSceneInfo(path, images, eval, test_camera_count=8):
    try:
    ...
    if eval:
        # Evenly distribute test cameras including first and last frame
        total_cams = len(cam_infos)
        test_camera_count = min(test_camera_count, total_cams)  # Cap at total available
        
        if test_camera_count <= 0:
            test_indices = []
        elif test_camera_count == 1:
            test_indices = [0]  # Just first frame
        elif test_camera_count == 2:
            test_indices = [0, total_cams - 1]  # First and last
        else:
            # First, last, and evenly distributed middle cameras
            test_indices = [0]  # Start with first frame
            step = (total_cams - 1) / (test_camera_count - 1)
            for i in range(1, test_camera_count):
                test_indices.append(int(round(i * step)))
        
        test_indices_set = set(test_indices)
        test_cam_infos = [cam_infos[idx] for idx in test_indices]
        train_cam_infos = [c for idx, c in enumerate(cam_infos) if idx not in test_indices_set]
    else:
    ...
    if not os.path.exists(ply_path):
        log.log("Converting point3d.bin to .ply (first time only)...")
        try:
