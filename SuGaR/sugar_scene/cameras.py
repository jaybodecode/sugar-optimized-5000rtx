...
def load_gs_cameras(source_path, gs_output_path, image_resolution=1, 
    ...
    for cam_idx in range(len(camera_transforms)):
        ...
        # Calculate image dimensions and downscale factor
        if load_gt_images:
            image = Image.open(image_path)
            orig_w, orig_h = image.size
            image.close()  # Close immediately, we'll reload on-demand
        else:
        ...
        # Store loading parameters for lazy loading instead of loading image
        loading_params = {
            'image_path': image_path if load_gt_images else None,
...
class GSCamera(torch.nn.Module):
    ...
    Implements lazy image loading to save VRAM:
    - Images stored on disk/CPU until needed
    - LRU cache keeps last 10 images in memory
    - Saves ~5GB VRAM for 161-image dataset
    """
    
    # Class-level LRU cache for lazy-loaded images (shared across all cameras)
    _image_cache = {}
    _cache_size = 5  # Tuned for NTFS mount (~10-20ms load). On native ext4/NVMe, could reduce to 2-3.
    ...
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask,
        ...
        # Store dimensions, but don't load image yet (lazy loading)
        ...
        # For backward compatibility: if image provided directly, store it
        self._cached_image = None
        if image is not None:
            self._cached_image = image.clamp(0.0, 1.0).to(self.data_device)
            if gt_alpha_mask is not None:
                self._cached_image *= gt_alpha_mask.to(self.data_device)
            else:
                self._cached_image *= torch.ones((1, self.image_height, self.image_width), device=self.data_device)
    ...
    def original_image(self):
        """Lazy-load ground truth image on first access.
        
        Uses class-level LRU cache to keep last 10 images in VRAM.
        Saves ~5GB by not loading all 161 images at startup.
        """
        # If image was provided directly at init (backward compatibility)
        if self._cached_image is not None:
            return self._cached_image
            
        # If no loading params, can't lazy load
        if self.loading_params is None or self.loading_params['image_path'] is None:
            raise ValueError(f"Cannot load image for camera {self.image_name}: no loading parameters")
        
        cache_key = self.image_name
        
        # Check cache
        if cache_key in GSCamera._image_cache:
            # Move to end of LRU order (most recently used)
            GSCamera._cache_order.remove(cache_key)
            GSCamera._cache_order.append(cache_key)
            return GSCamera._image_cache[cache_key]
        
        # Load image from disk
        image_path = self.loading_params['image_path']
        resolution = self.loading_params['resolution']
        white_background = self.loading_params['white_background']
        ...
        gt_image *= torch.ones((1, self.image_height, self.image_width), device=self.data_device)
        
        # Add to cache
        GSCamera._image_cache[cache_key] = gt_image
        GSCamera._cache_order.append(cache_key)
        
        # Evict oldest if cache full
        while len(GSCamera._cache_order) > GSCamera._cache_size:
            oldest_key = GSCamera._cache_order.pop(0)
            del GSCamera._image_cache[oldest_key]
        
        return gt_image
    
    @staticmethod
    def clear_image_cache():
        """Clear the image cache to free VRAM."""
        GSCamera._image_cache.clear()
        GSCamera._cache_order.clear()
