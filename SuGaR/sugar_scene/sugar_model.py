    ...
    def __init__(
        ...
        *args, **kwargs) -> None:
        ...
        if keep_track_of_knn:
            import time
            import gc
            CONSOLE.print(f"  [cyan]Computing KNN K={knn_to_track} for {len(points):,} points...[/cyan]")
            t_knn_start = time.time()
            
            # Clear cache before KNN computation
            ...
            elapsed = time.time() - t_knn_start
            vram_gb = torch.cuda.memory_allocated() / (1024**3) if torch.cuda.is_available() else 0
            CONSOLE.print(f"  [green]✓ KNN K={knn_to_track} completed in {elapsed:.1f}s (VRAM: {vram_gb:.1f}GB)[/green]")
        ...
        if not self.binded_to_surface_mesh:
            ...
            if initialize:
                import time
                CONSOLE.print(f"  [cyan]Initializing Gaussian radiuses (KNN K=4 for {n_points:,} points)...[/cyan]")
                t_radius_start = time.time()
                radiuses = _initialize_radiuses_gauss_rasterizer(self,)
                elapsed = time.time() - t_radius_start
                vram_gb = torch.cuda.memory_allocated() / (1024**3) if torch.cuda.is_available() else 0
                CONSOLE.print(f"  [green]✓ Radius initialization completed in {elapsed:.1f}s (VRAM: {vram_gb:.1f}GB)[/green]")
    ...
    def reset_neighbors(self, knn_to_track:int=None):
        if self.binded_to_surface_mesh:
            ...
            with torch.no_grad():
                ...
                # Delete old KNN tensors to prevent memory leak
                if hasattr(self, 'knn_dists'):
                    del self.knn_dists
                if hasattr(self, 'knn_idx'):
                    del self.knn_idx
                
                # CRITICAL: Clear cache BEFORE allocating new KNN to prevent fragmentation
                # This ensures we have maximum contiguous memory for the KNN computation
                torch.cuda.empty_cache()
                import gc
                gc.collect()
                
                mem_after_delete = torch.cuda.memory_allocated() / (1024**3) if torch.cuda.is_available() else 0
                ...
                mem_after_knn = torch.cuda.memory_allocated() / (1024**3) if torch.cuda.is_available() else 0
                
                # Explicitly delete the temporary knns object (holds references to .dists and .idx)
                del knns
                
                # Final cleanup after KNN computation
                torch.cuda.empty_cache()
                gc.collect()
                
                mem_final = torch.cuda.memory_allocated() / (1024**3) if torch.cuda.is_available() else 0
                
                # Always log memory changes during neighbor reset for debugging
                CONSOLE.print(f"[yellow]📊 KNN Memory:[/yellow] Before: {mem_before:.2f}GB | After del: {mem_after_delete:.2f}GB | After KNN: {mem_after_knn:.2f}GB | Final: {mem_final:.2f}GB | [bold]Delta: {mem_final-mem_before:+.2f}GB[/bold]")
