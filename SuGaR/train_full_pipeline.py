...
if __name__ == "__main__":
    ...
    # VRAM Optimization
    parser.add_argument('--full_res_normals', type=str2bool, default=False,
                        help='Use full resolution for depth-normal consistency. '
                        'Default False (half-res saves 4-5GB VRAM). '
                        'Set True if you have 24GB+ VRAM.')
    ...
    test_iterations_str = ' '.join(map(str, args.test_iterations))
    checkpoint_milestones_str = ' '.join(map(str, args.checkpoint_milestones))
    os.system(
        f"python train.py \
            -s {args.scene_path} \
            -c {gs_checkpoint_dir} \
            ...
            --checkpoint_milestones {checkpoint_milestones_str} \
            --test_iterations {test_iterations_str}"
    )