...
def coarse_training_with_density_regularization(args):
    ...
    # ====================Scene Info Display====================
    info_table = Table(title="Training Configuration", show_header=True, header_style="bold cyan")
    info_table.add_column("Property", style="cyan", width=30)
    info_table.add_column("Value", style="green", width=25)
    
    info_table.add_row("Training Cameras", f"{len(nerfmodel.training_cameras)}")
    info_table.add_row("Initial Gaussians", f"{sugar.n_points:,}")
    info_table.add_row("Iterations", f"{num_iterations:,}")
    info_table.add_row("Loss Function", loss_function)
    info_table.add_row("SH Levels", str(sh_levels))
    info_table.add_row("Resolution Factor", str(downscale_resolution_factor))
    info_table.add_row("Scene Extent", f"{cameras_spatial_extent:.2f}")
    ...
    if torch.cuda.is_available():
        ...
        import gc
        gc.collect()
        CONSOLE.print("[green]✓ VRAM cache cleared[/green]")
    ...
    if initialize_from_trained_3dgs:
        iteration = iteration_to_load  # Resume from checkpoint iteration
    ...
    with Progress(
        ...
        TextColumn("L:[magenta]{task.fields[loss]:.3f}[/magenta]"),
        ...
        progress.update(task, completed=iteration, loss=0.0, vram=vram_str, ram=ram_str)
        
        # Initialize timing for speed tracking
        ...
        for batch in range(9_999_999):
            ...
            for i in range(0, train_num_images, train_num_images_per_batch):
                ...
                if (
                    ...
                    progress.console.print("\n[cyan]Pruning gaussians with low-opacity for further optimization...[/cyan]")
                    ...
                    progress.console.print(f'[green]Pruning finished: {sugar.n_points} gaussians left.[/green]')
            ...
            if not no_rendering:
                ...
                if enforce_entropy_regularization and iteration > start_entropy_regularization_from and iteration < end_entropy_regularization_at:
                    if iteration == start_entropy_regularization_from + 1:
                        progress.console.print("\n[cyan]Starting entropy regularization.[/cyan]")
                    if iteration == end_entropy_regularization_at - 1:
                        progress.console.print("\n[cyan]Stopping entropy regularization.[/cyan]")
            ...
            with torch.no_grad():
                if (not no_rendering) and (iteration < densify_until_iter):
                    ...
                    if iteration > densify_from_iter and iteration % densification_interval == 0:
                        ...
                        progress.console.print(f"[cyan]Gaussians densified and pruned. New number: {len(sugar.points)}[/cyan]")
                        
                        if regularize and (iteration > regularize_from) and (iteration >= start_reset_neighbors_from):
                            sugar.reset_neighbors()
                            progress.console.print("[cyan]Neighbors reset.[/cyan]")
                    
                    if iteration % opacity_reset_interval == 0:
                        gaussian_densifier.reset_opacity()
                        progress.console.print("[cyan]Opacity reset.[/cyan]")
            ...
            ema_loss_for_log = 0.4 * loss.detach().item() + 0.6 * ema_loss_for_log
            ...
            if iteration % 10 == 0:
                ...
                speed_str = f"{10.0/elapsed_time:.1f}it/s" if elapsed_time > 0 else "--"
                iter_start_time = time.time()  # Reset timer
                
                progress.update(task, completed=iteration, loss=ema_loss_for_log, vram=vram_str, ram=ram_str, speed=speed_str)
            
            # Print detailed stats periodically
            if iteration==1 or iteration % print_loss_every_n_iterations == 0:
                with torch.no_grad():
                    progress.console.print(f"\n[bold cyan]Iteration {iteration}/{num_iterations}[/bold cyan] - Loss: [magenta]{loss:.7f}[/magenta]")
                    progress.console.print(f"  Points: {sugar.points.mean():.3f} ± {sugar.points.std():.3f}")
                    progress.console.print(f"  Scales: {sugar.scaling.mean():.4f} ± {sugar.scaling.std():.4f}")
                    progress.console.print(f"  Opacities: {sugar.strengths.mean():.2f} ± {sugar.strengths.std():.2f}")
                    if regularize_sdf and iteration > start_sdf_regularization_from:
                        progress.console.print(f"  SDF gaussians sampled: {n_gaussians_in_sampling}")
                t0 = time.time()
            ...
            if (iteration % save_model_every_n_iterations == 0) or (iteration in save_milestones):
                ...
                progress.console.print(f"[green]✓ Checkpoint saved: {iteration}.pt ({size_mb:.1f} MB)[/green]")
            ...
            if do_sh_warmup and (iteration > 0) and (current_sh_levels < sh_levels) and (iteration % sh_warmup_every == 0):
                current_sh_levels += 1
                progress.console.print(f"[cyan]SH levels increased to {current_sh_levels}[/cyan]")
            ...
            if do_resolution_warmup and (iteration > 0) and (current_resolution_factor > 1) and (iteration % resolution_warmup_every == 0):
                ...
                height = nerfmodel.training_cameras.ns_cameras.height[0].item()
                width = nerfmodel.training_cameras.ns_cameras.width[0].item()
                progress.console.print(f"[cyan]Resolution scaled to {width} x {height}[/cyan]")
    ...
    # Training complete
    CONSOLE.print(f"\\n[bold green]Training finished after {num_iterations} iterations[/bold green]")
    CONSOLE.print(f"Final loss: [magenta]{loss.detach().item():.7f}[/magenta]")
    CONSOLE.print("\\nSaving final model...")
    model_path = os.path.join(sugar_checkpoint_path, f'{iteration}.pt')
    sugar.save_model(path=model_path,
                    train_losses=train_losses,
    ...
    size_mb = os.path.getsize(model_path) / (1024**2)
    CONSOLE.print(f"[green]\u2713 Final model saved: {iteration}.pt ({size_mb:.1f} MB)[/green]")
    return model_path