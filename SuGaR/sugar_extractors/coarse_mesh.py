...
def extract_mesh_from_coarse_sugar(args):
    ...
    params_table = Table(title="[bold cyan]Mesh Extraction Configuration[/bold cyan]", show_header=True, header_style="bold cyan")
    params_table.add_column("Setting", style="cyan", width=35)
    params_table.add_column("Value", style="green")
    
    params_table.add_row("Source path", str(source_path))
    params_table.add_row("Gaussian Splatting Checkpoint", str(gs_checkpoint_path))
    params_table.add_row("Coarse model Checkpoint", str(sugar_checkpoint_path))
    params_table.add_row("Mesh output path", str(mesh_output_dir))
    params_table.add_row("Surface levels", str(surface_levels))
    params_table.add_row("Decimation targets", str(decimation_targets))
    params_table.add_row("Project mesh on surface", str(project_mesh_on_surface_points))
    params_table.add_row("Use custom bbox", str(use_custom_bbox))
    params_table.add_row("Use eval split", str(use_train_test_split))
    params_table.add_row("GPU", str(args.gpu))
    params_table.add_row("Use centers to extract mesh", str(use_centers_to_extract_mesh))
    params_table.add_row("Use marching cubes", str(use_marching_cubes))
    params_table.add_row("Use vanilla 3DGS", str(use_vanilla_3dgs))
    ...
    CONSOLE.print("[bold cyan]Coarse model loaded.[/bold cyan]")
    
    # Create model parameters table
    model_params_table = Table(title="[bold cyan]Coarse Model Parameters[/bold cyan]", show_header=True, header_style="bold cyan")
    model_params_table.add_column("Parameter", style="cyan", width=40)
    model_params_table.add_column("Shape", style="yellow", width=25)
    model_params_table.add_column("Trainable", style="green")
    
    for name, param in sugar.named_parameters():
        model_params_table.add_row(name, str(tuple(param.shape)), str(param.requires_grad))
    ...
    with torch.no_grad():
        # Create pre-pruning opacity statistics table
        pre_prune_table = Table(title="[bold yellow]Pre-Pruning Statistics[/bold yellow]", show_header=True, header_style="bold yellow")
        pre_prune_table.add_column("Metric", style="cyan", width=30)
        pre_prune_table.add_column("Value", style="green")
        
        pre_prune_table.add_row("Number of Gaussians", f"{sugar.n_points:,}")
        pre_prune_table.add_row("Opacity Min", f"{sugar.strengths.min():.6f}")
        pre_prune_table.add_row("Opacity Max", f"{sugar.strengths.max():.6f}")
        pre_prune_table.add_row("Opacity Mean", f"{sugar.strengths.mean():.6f}")
        
        CONSOLE.print(pre_prune_table)
        CONSOLE.print()
        
        # Create quantiles table
        quantiles_table = Table(title="[bold yellow]Opacity Quantiles[/bold yellow]", show_header=True, header_style="bold yellow")
        quantiles_table.add_column("Quantile", style="cyan", width=15)
        quantiles_table.add_column("Opacity", style="green")
        
        n_quantiles = 10
        for i in range(n_quantiles):
            quantiles_table.add_row(f"{i/n_quantiles:.1f}", f"{sugar.strengths.quantile(i/n_quantiles).item():.6f}")
        
        CONSOLE.print(quantiles_table)
        CONSOLE.print()
            
        CONSOLE.print("[bold magenta]Starting pruning low opacity gaussians...[/bold magenta]")
        sugar.drop_low_opacity_points(low_opacity_gaussian_pruning_threshold)

        # Create post-pruning statistics table
        post_prune_table = Table(title="[bold green]Post-Pruning Statistics[/bold green]", show_header=True, header_style="bold green")
        post_prune_table.add_column("Metric", style="cyan", width=30)
        post_prune_table.add_column("Value", style="green")
        
        post_prune_table.add_row("Number of Gaussians", f"{sugar.n_points:,}")
        post_prune_table.add_row("Opacity Min", f"{sugar.strengths.min():.6f}")
        post_prune_table.add_row("Opacity Max", f"{sugar.strengths.max():.6f}")
        post_prune_table.add_row("Opacity Mean", f"{sugar.strengths.mean():.6f}")
        
        CONSOLE.print(post_prune_table)
        CONSOLE.print()
        
        # Create post-pruning quantiles table
        post_quantiles_table = Table(title="[bold green]Post-Pruning Opacity Quantiles[/bold green]", show_header=True, header_style="bold green")
        post_quantiles_table.add_column("Quantile", style="cyan", width=15)
        post_quantiles_table.add_column("Opacity", style="green")
        
        n_quantiles = 10
        for i in range(n_quantiles):
            post_quantiles_table.add_row(f"{i/n_quantiles:.1f}", f"{sugar.strengths.quantile(i/n_quantiles).item():.6f}")
        
        CONSOLE.print(post_quantiles_table)
        CONSOLE.print()
    ...
    # VRAM Optimization: Default 0.5 resolution factor + faces_per_pixel=5 saves ~6-8GB VRAM
    # (24GB → ~15GB) with minimal quality loss. Use --mesh_extraction_resolution_factor 1.0 for full-res on 32GB+ GPUs.
    resolution_factor = args.mesh_extraction_resolution_factor if hasattr(args, 'mesh_extraction_resolution_factor') else 0.5
    faces_per_pixel = 5  # Reduced from 10 (saves ~2-3GB, minimal impact on mesh quality)
    max_faces_per_bin = 50_000

    mesh_height = int(sugar.image_height * resolution_factor)
    mesh_width = int(sugar.image_width * resolution_factor)

    mesh_raster_settings = RasterizationSettings(
        image_size=(mesh_height, mesh_width),
        blur_radius=0.0, 
            ...
            for surface_level in surface_levels:
                ...
                if fg_points.shape[0] > 0:
                    ...
                    with Progress(
                        ...
                        task = progress.add_task(
                            "Computing Poisson mesh (foreground)...",
                            total=None,
                            vram=get_vram_str()
                        )
                    ...
                    if vertices_density_quantile > 0.:
                        CONSOLE.print("[yellow]Removing vertices with low densities...[/yellow]")
                    ...
                    n_fg_verts = len(o3d_fg_mesh.vertices) if o3d_fg_mesh is not None else 0
                    n_fg_faces = len(o3d_fg_mesh.triangles) if o3d_fg_mesh is not None else 0
                    CONSOLE.print(f"[green]✓ Foreground mesh complete ({n_fg_verts:,} vertices, {n_fg_faces:,} faces)[/green]")
                else:
                ...
                if bg_points.shape[0] > 0:
                    ...
                    with Progress(
                        ...
                        task = progress.add_task(
                            "Computing Poisson mesh (background)...",
                            total=None,
                            vram=get_vram_str()
                        )
                    ...
                    n_bg_verts = len(o3d_bg_mesh.vertices) if o3d_bg_mesh is not None else 0
                    n_bg_faces = len(o3d_bg_mesh.triangles) if o3d_bg_mesh is not None else 0
                    CONSOLE.print(f"[green]✓ Background mesh complete ({n_bg_verts:,} vertices, {n_bg_faces:,} faces)[/green]")
                else:
                ...
                for decimation_target in decimation_targets:
                    ...
                    if decimate_mesh:
                        if o3d_fg_mesh is not None:
                            with Progress(
                                ...
                                task = progress.add_task(
                                    f"Decimating foreground mesh to {decimation_target:,} vertices...",
                                    total=None,
                                    vram=get_vram_str()
                                )
                                decimated_o3d_fg_mesh = o3d_fg_mesh.simplify_quadric_decimation(decimation_target)
                            n_dec_fg_verts = len(decimated_o3d_fg_mesh.vertices)
                            n_dec_fg_faces = len(decimated_o3d_fg_mesh.triangles)
                            CONSOLE.print(f"[green]✓ Foreground decimation complete ({n_dec_fg_verts:,} vertices, {n_dec_fg_faces:,} faces)[/green]")
                        else:
                        ...
                        if o3d_bg_mesh is not None:
                            with Progress(
                                ...
                                task = progress.add_task(
                                    f"Decimating background mesh to {decimation_target:,} vertices...",
                                    total=None,
                                    vram=get_vram_str()
                                )
                                decimated_o3d_bg_mesh = o3d_bg_mesh.simplify_quadric_decimation(decimation_target)
                            n_dec_bg_verts = len(decimated_o3d_bg_mesh.vertices)
                            n_dec_bg_faces = len(decimated_o3d_bg_mesh.triangles)
                            CONSOLE.print(f"[green]✓ Background decimation complete ({n_dec_bg_verts:,} vertices, {n_dec_bg_faces:,} faces)[/green]")
                        else:
                    ...
                    if project_mesh_on_surface_points:
                        with Progress(
                            ...
                            task = progress.add_task(
                                "Projecting mesh onto surface points...",
                                total=None,
                                vram=get_vram_str()
                            )
                        ...
                        n_proj_verts = len(decimated_o3d_mesh.vertices)
                        n_proj_faces = len(decimated_o3d_mesh.triangles)
                        CONSOLE.print(f"[green]✓ Mesh projection complete ({n_proj_verts:,} vertices, {n_proj_faces:,} faces)[/green]")
